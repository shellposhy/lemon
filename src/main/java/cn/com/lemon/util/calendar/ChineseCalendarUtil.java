...
public class ChineseCalendarUtil {
	...
	public static void main(String[] args) {
		ChineseCalendarUtil l = new ChineseCalendarUtil(System.currentTimeMillis());
	...
	private synchronized void findFestival() {
		...
		Matcher m;
		for (int i = 0; i < ChineseCalendarUtil.sFtv.length; i++) {
			m = ChineseCalendarUtil.sFreg.matcher(ChineseCalendarUtil.sFtv[i]);
			if (m.find()) {
				if (sM == ChineseCalendarUtil.toInt(m.group(1)) && sD == ChineseCalendarUtil.toInt(m.group(2))) {
		...
		for (int i = 0; i < ChineseCalendarUtil.lFtv.length; i++) {
			m = ChineseCalendarUtil.sFreg.matcher(ChineseCalendarUtil.lFtv[i]);
			if (m.find()) {
				if (lM == ChineseCalendarUtil.toInt(m.group(1)) && lD == ChineseCalendarUtil.toInt(m.group(2))) {
		...
		// 月周节日
		int w, d;
		for (int i = 0; i < ChineseCalendarUtil.wFtv.length; i++) {
			m = ChineseCalendarUtil.wFreg.matcher(ChineseCalendarUtil.wFtv[i]);
			if (m.find()) {
				if (this.getSolarMonth() == ChineseCalendarUtil.toInt(m.group(1))) {
					w = ChineseCalendarUtil.toInt(m.group(2));
					d = ChineseCalendarUtil.toInt(m.group(3));
	...
	private static int getLunarLeapMonth(int lunarYear) {
		...
		int leapMonth = ChineseCalendarUtil.lunarInfo[lunarYear - 1900] & 0xf;
	...
	private static int getLunarLeapDays(int lunarYear) {
		...
		// 若该年没有闰月,返回0
		return ChineseCalendarUtil.getLunarLeapMonth(lunarYear) > 0
				? ((ChineseCalendarUtil.lunarInfo[lunarYear - 1899] & 0xf) == 0xf ? 30 : 29) : 0;
	}
	...
	private static int getLunarYearDays(int lunarYear) {
		...
		for (int i = 0x8000; i > 0x8; i >>= 1) {
			daysInLunarYear += ((ChineseCalendarUtil.lunarInfo[lunarYear - 1900] & i) != 0) ? 1 : 0;
		}
		// 加上闰月天数
		daysInLunarYear += ChineseCalendarUtil.getLunarLeapDays(lunarYear);
	...
	private static int getLunarMonthDays(int lunarYear, int lunarMonth) {
		// 数据表中,每个农历年用16bit来表示,
		// 前12bit分别表示12个月份的大小月,最后4bit表示闰月
		int daysInLunarMonth = ((ChineseCalendarUtil.lunarInfo[lunarYear - 1900] & (0x10000 >> lunarMonth)) != 0) ? 30
				: 29;
	...
	public static synchronized int getUTCDay(Date date) {
		ChineseCalendarUtil.makeUTCCalendar();
		synchronized (utcCal) {
			utcCal.clear();
	...
	private static synchronized void makeUTCCalendar() {
		if (ChineseCalendarUtil.utcCal == null) {
			ChineseCalendarUtil.utcCal = new GregorianCalendar(TimeZone.getTimeZone("UTC"));
		}
	}
	...
	private static int getSolarTermDay(int solarYear, int index) {
		...
		l = l + ChineseCalendarUtil.UTC(1900, 0, 6, 2, 5, 0);
		return ChineseCalendarUtil.getUTCDay(new Date(l));
	}
	 ...
	 * @param TimeInMillis
	 */
	public ChineseCalendarUtil(long TimeInMillis) {
	...
	private void init(long TimeInMillis) {
		...
		this.lunarYear = 1900;
		int daysInLunarYear = ChineseCalendarUtil.getLunarYearDays(this.lunarYear);
		while (this.lunarYear < 2100 && offset >= daysInLunarYear) {
			offset -= daysInLunarYear;
			daysInLunarYear = ChineseCalendarUtil.getLunarYearDays(++this.lunarYear);
		}
		int lunarMonth = 1;
		int leapMonth = ChineseCalendarUtil.getLunarLeapMonth(this.lunarYear);
		...
		while (lunarMonth < 13 && offset > 0) {
			if (isLeap && leapDec) {
				daysInLunarMonth = ChineseCalendarUtil.getLunarLeapDays(this.lunarYear);
				leapDec = false;
			} else {
				daysInLunarMonth = ChineseCalendarUtil.getLunarMonthDays(this.lunarYear, lunarMonth);
			}
	...
	private void getCyclicalData() {
		...
		int term2 = ChineseCalendarUtil.getSolarTermDay(solarYear, 2); // 立春日期
		// 依节气调整二月分的年柱, 以立春为界
		...
		int firstNode = ChineseCalendarUtil.getSolarTermDay(solarYear, solarMonth * 2); // 传回当月「节」为几日开始
		// 依节气月柱, 以「节」为界
		...
		cyclicalDay = (int) (ChineseCalendarUtil.UTC(solarYear, solarMonth, solarDay, 0, 0, 0) / 86400000 + 25567 + 10)
				% 60;
	...
	public String getAnimalString() {
		return ChineseCalendarUtil.Animals[(this.lunarYear - 4) % 12];
	}
	...
	public String getTermString() {
		// 二十四节气
		String termString = "";
		if (ChineseCalendarUtil.getSolarTermDay(solarYear, solarMonth * 2) == solarDay) {
			termString = ChineseCalendarUtil.solarTerm[solarMonth * 2];
		} else if (ChineseCalendarUtil.getSolarTermDay(solarYear, solarMonth * 2 + 1) == solarDay) {
			termString = ChineseCalendarUtil.solarTerm[solarMonth * 2 + 1];
		}
		return termString;
	}
	...
	public int getTiananY() {
		return ChineseCalendarUtil.getTianan(this.cyclicalYear);
	}
	...
	public int getTiananM() {
		return ChineseCalendarUtil.getTianan(this.cyclicalMonth);
	}
	...
	public int getTiananD() {
		return ChineseCalendarUtil.getTianan(this.cyclicalDay);
	}
	 ...
	 */
	public int getDeqiY() {
		return ChineseCalendarUtil.getDeqi(this.cyclicalYear);
	}
	 ...
	 */
	public int getDeqiM() {
		return ChineseCalendarUtil.getDeqi(this.cyclicalMonth);
	}
	 ...
	 */
	public int getDeqiD() {
		return ChineseCalendarUtil.getDeqi(this.cyclicalDay);
	}
	...
	public String getCyclicaYear() {
		return ChineseCalendarUtil.getCyclicalString(this.cyclicalYear);
	}
	...
	public String getCyclicaMonth() {
		return ChineseCalendarUtil.getCyclicalString(this.cyclicalMonth);
	}
	...
	public String getCyclicaDay() {
		return ChineseCalendarUtil.getCyclicalString(this.cyclicalDay);
	}
	...
	public String getLunarDayString() {
		return ChineseCalendarUtil.getLunarDayString(this.lunarDay);
	}
	...
	public String getLunarMonthString() {
		return (this.isLeap() ? "闰" : "") + ChineseCalendarUtil.getLunarMonthString(this.lunarMonth);
	}
	...
	public String getLunarYearString() {
		return ChineseCalendarUtil.getLunarYearString(this.lunarYear);
	}
	...
	private static String getCyclicalString(int cyclicalNumber) {
		return ChineseCalendarUtil.Tianan[ChineseCalendarUtil.getTianan(cyclicalNumber)]
				+ ChineseCalendarUtil.Deqi[ChineseCalendarUtil.getDeqi(cyclicalNumber)];
	}
	...
	private static String getLunarYearString(int lunarYear) {
		return ChineseCalendarUtil.getCyclicalString(lunarYear - 1900 + 36);
	}
	...
	private static String getLunarMonthString(int lunarMonth) {
		...
		if (lunarMonth == 1) {
			lunarMonthString = ChineseCalendarUtil.lunarString2[4];
		} else {
			if (lunarMonth > 9)
				lunarMonthString += ChineseCalendarUtil.lunarString2[1];
			if (lunarMonth % 10 > 0)
				lunarMonthString += ChineseCalendarUtil.lunarString1[lunarMonth % 10];
		}
	...
	private static String getLunarDayString(int lunarDay) {
		...
		int i1 = lunarDay / 10;
		int i2 = lunarDay % 10;
		String c1 = ChineseCalendarUtil.lunarString2[i1];
		String c2 = ChineseCalendarUtil.lunarString1[i2];
		if (lunarDay < 11)
			c1 = ChineseCalendarUtil.lunarString2[0];
		if (i2 == 0)
			c2 = ChineseCalendarUtil.lunarString2[1];
		return c1 + c2;
	}
}