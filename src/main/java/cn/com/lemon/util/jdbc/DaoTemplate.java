...
import cn.com.lemon.util.jdbc.annotation.Column;
import cn.com.lemon.util.jdbc.annotation.Id;
import cn.com.lemon.util.jdbc.annotation.Table;
 ...
 * 
 * <p>
 * Value对象的属性必须是下列类型才能够支持：int, long, String, BigDecimal, java.util.Date, float,
 * double, boolean, enum。其中：
 * <ul>
...
public abstract class DaoTemplate<T> extends JdbcDaoSupport {
	...
	private void initByAnnotation() {
		...
		tableName = (table == null) ? entityClass.getSimpleName().toUpperCase() : table.name().toUpperCase();
		...
		for (Method method : methodArray) {
			...
			Method setter = null;
			try {
				setter = entityClass.getMethod("set" + propertyNameWithFirstLetterUpper, retClass);
			...
			} else if (retClass.equals(Long.class) || retClass.equals(long.class)) {
				f.setJavaType(FieldJavaType.LONG);
			} else if (retClass.equals(Integer.class) || retClass.equals(int.class)) {
			...
			} else if (retClass.equals(Boolean.class) || retClass.equals(boolean.class)) {
				f.setJavaType(FieldJavaType.BOOLEAN);
			} else if (retClass.equals(Double.class) || retClass.equals(double.class)) {
				f.setJavaType(FieldJavaType.DOUBLE);
			} else if (retClass.equals(Float.class) || retClass.equals(float.class)) {
	...
	private Class<?> getSuperClassGenricType() {
		...
		if (!(genType instanceof ParameterizedType)) {
			...
			throw new RuntimeException(clazz.getSimpleName() + "没有通过泛型指定Value对象");
		}
	...
	private void setValue(PreparedStatement ps, int index, T obj, Field f) throws SQLException {
		Object param = null;
		try {
			...
			throw new RuntimeException("获取属性" + f.getJavaName() + "的值时失败", e.getTargetException());
		}
	...
	private void getValue(ResultSet rs, int index, Object obj, Field f) throws SQLException {
		Object value = null;
		switch (f.getJavaType()) {
		case BIGDECIMAL:
		...
		try {
			...
			throw new RuntimeException("设置属性" + f.getJavaName() + "的值时失败", e.getTargetException());
		}
	}

	private void getValue(ResultSet rs, int index, BaseDataObject obj, String colName) throws SQLException {
	...
	public void add(final T[] objects) {
		...
		for (final Object[] batch : batchs) {
			getJdbcTemplate().batchUpdate(fullInsertSql, new BatchPreparedStatementSetter() {
	...
	public boolean update(final T object) {
		...
		int x = getJdbcTemplate().update(fullUpdateSql, new PreparedStatementSetter() {
			public void setValues(PreparedStatement ps) throws SQLException {
				int index = 1;
				for (Field f : fields) {
					if (f.isPK()) {
	...
	public int update(final T[] objects) {
		...
		int count = 0;
		for (final Object[] batch : batchs) {
			int[] x = getJdbcTemplate().batchUpdate(fullUpdateSql, new BatchPreparedStatementSetter() {
				public int getBatchSize() {
					return batch.length;
				}
	...
	protected List<T> queryByCondition(String conditionSql, Object[] params) {
		return getJdbcTemplate().query("SELECT " + allFields + " FROM " + tableName + " WHERE " + conditionSql, params,
				fullRowMapper);
	}

	/**
	 * 使用sql语句进行查询。当Value对象继承了<code>EntityValue</code>时，会把无法匹配的 字段存入
	 * <code>EntityValue</code>的props中。
	 * 
	 * @param sql
	 *            sql语句
	 * @param params
	...
	protected int deleteByCondition(String conditionSql, Object[] params) {
		return getJdbcTemplate().update("DELETE FROM " + tableName + " WHERE  " + conditionSql, params);
	}
	...
	protected List<T> queryByProperty(String propertyName, Object value) {
		return getJdbcTemplate().query("SELECT " + allFields + " FROM " + tableName + " WHERE " + propertyName + " = ?",
				new Object[] { value }, fullRowMapper);
	}
	...
	public List<T> queryAll() {
		return getJdbcTemplate().query("SELECT " + allFields + " FROM " + tableName, fullRowMapper);
	}
	...
	public boolean deleteById(Object id) {
		...
		int x = getJdbcTemplate().update("DELETE FROM " + tableName + " WHERE " + PK.getColumnName() + "=?",
				new Object[] { id });
		return x == 1;
	}
	...
	public int deleteById(Object[] ids) {
		...
		for (final Object[] batchIds : idList) {
			int[] x = getJdbcTemplate().batchUpdate(sql, new BatchPreparedStatementSetter() {
				public int getBatchSize() {
	...
	protected int deleteByProperty(String propertyName, Object value) {
		...
		return getJdbcTemplate().update("DELETE FROM " + tableName + " WHERE " + propertyName + "=?",
				new Object[] { value });
	}
	 ...
	 * 这时使用这个RowMapper，可以不用读取ResultSetMetaData。 这个变量子类不应该用，因此定义为私有。
	 */
	...
	/**
	 * 字段映射，子类直接使用，假设选取了这个表的部分字段，选取了那些字段从rs中获取。 使用这个RowMapper，当Value对象继承了
	 * <code>EntityValue</code>时，会把无法匹配的 字段存入<code>EntityValue</code>的props中。
	 */
	@SuppressWarnings("rawtypes")
	...
	protected RowMapper mapRowMapper = new RowMapper() {
		/**
		 * 遍历ResultSet，将数据压制到Map<String, Object>中，并返回 应用场景：适用于自己拼写sql的情况
		 * 注意:有别名时的情况
		 */
		public Object mapRow(ResultSet rs, int Index) throws SQLException {
